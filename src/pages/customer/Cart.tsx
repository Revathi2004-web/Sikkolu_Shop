...
const Cart = () => {
  const { cart, updateCartQty, removeFromCart, paymentMethods } = useStore();
  const { t } = useLanguage();
  ...
  return (
    <div className="min-h-screen bg-background pb-8">
      <header className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-xl font-serif font-bold">{t.yourCart}</h1>
      </header>

      {cart.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          <ShoppingBag className="w-16 h-16 mb-4" />
          <p className="text-lg font-serif">{t.cartEmpty}</p>
          <Button variant="outline" className="mt-4 rounded-xl" onClick={() => navigate('/store')}>{t.continueShopping}</Button>
        </div>
      ) : (
        <div className="px-4 pt-4">
          ...
          {paymentMethods.length > 0 && (
            <div className="mb-4">
              <h3 className="text-sm font-semibold text-muted-foreground mb-2">{t.paymentOptions}</h3>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            ...
            <Button className="w-full h-14 text-lg rounded-xl font-semibold" onClick={() => navigate('/checkout')}>
              {t.placeOrder}
            </Button>
          </div>
        </div>
